...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<SP> }) {
  const user = await requireSection("hisobot");
  const sp = await searchParams;

  // Manba (soha) kesimi + rol doirasi — boshqaruv paneli bilan YAGONA funksiyadan
  // (`dashboard/scope.ts`), aks holda ikki sahifada jimgina ajralib ketardi.
  const {
    scope,
    soha,
    sohaList,
    ...
    ownSources,
    effectiveSourceIds,
  } = await resolveDashboardScope(user, str(sp.soha) || undefined);
  ...
  // ⚠️ HUDUD/TUMAN qatorlari respublika darajasidagi tashkilotlarni ("Markaziy
  // apparat") O'Z ICHIGA OLMAYDI — ular alohida qatorda hisoblanadi. Lekin ularning
  // obyektlari kadastr prefiksi orqali oddiy hududlarga tarqalgan, ya'ni oddiy
  // `?region=<id>` havolasi ularni QAYTA olib kirardi va ro'yxatdagi son jadvaldagidan
  // katta chiqardi (jonli o'lchov: Toshkent sh. — 47 ↔ 71, 24 ta obyekt Agentlik
  // markaziy apparatiniki). Shu sabab har bir hudud/tuman havolasida `hududiy=1`.
  //
  // ⚠️ JAMI qatoriga va "Markaziy apparat" (`tashkilot=`) qatoriga QO'SHILMAYDI —
  // birinchisi butun doira bo'yicha, ikkinchisi aynan o'sha tashkilot bo'yicha.
  const regionScope = (regionId: string) => `region=${regionId}&hududiy=1`;
  const districtScope = (regionId: string, districtId: string) =>
    `region=${regionId}&district=${districtId}&hududiy=1`;
  ...
  const dashHref = (expandRegionId?: string) => {
    ...
    return `/dashboard/hisobot${parts.length ? `?${parts.join("&")}` : ""}`;
  };
          ...
          <table className="w-full border-separate border-spacing-0 text-sm">
            ...
            <tbody>
              ...
              {catRows.map((r, i) => {
                ...
                const rowScope = national ? `tashkilot=${r.regionId}` : regionScope(r.regionId);
                return (
                    ...
                    {expanded
                      ? districts.map((d) => (
                          <CategoryTableRow
                            ...
                            zebra={DISTRICT_BG}
                            objHref={objHref}
                            scope={districtScope(r.regionId, d.regionId)}
                            nf={nf}
                            km={km}
                            lead={<span className="text-slate-300">·</span>}
                            label={
                              <Link
                                href={objHref(districtScope(r.regionId, d.regionId))}
                      ...
                      {expanded
                        ? districtUtility.map((d) => (
                            <UtilityTableRow
                              ...
                              nf={nf}
                              km={km}
                              scopeQs={districtScope(r.regionId, d.regionId)}
                              objHref={objHref}
                              vacantSuffix={vacantSuffix}
                              lead={<span className="text-slate-300">·</span>}
                              label={
                                <Link
                                  href={objHref(`${districtScope(r.regionId, d.regionId)}&category=11${vacantSuffix}`)}
